        ...
        return this.each(function(e) {
            ...
            function init() { //初始化函数
                ...
                $('#d-cancleBtn').on('click', function(event) {
                    destroyScroll();
                    $('#datePlugin').fadeOut(200).html(''); // 隐藏日期选择器
                    document.getElementsByTagName('body')[0].removeEventListener('touchmove', cancleDefault, false);
                });
            }
            ...
            function renderDom() {
                var mark = '<div id="datemark" class="datemark"></div>';
                ...
                monthHtml += '</div>';
                $('#datePlugin').html(mark+mainHtml);
                // $("#datemark").click(function(event) {
                //     $('#datePlugin').fadeOut(200); // 隐藏日期选择器
                // });
                switch (opts.theme) {
                    case 'date':
